...
function pong() {
  ...
  */
  ...
  const updatescorePlayer = interval(5)
    ...
    .subscribe(() => {
      scoreEnemy += 1; ball.setAttribute('cx', '300'); xvelocity = 1; y_velocity = 1; text_enemy.textContent = "Enemy: " + scoreEnemy.toString()
  ...
  const updatescoreEnemy = interval(5)
    ...
    .subscribe(() => {
      scorePlayer += 1; ball.setAttribute('cx', '300'); xvelocity = 1; y_velocity = 1; newText.textContent = "Player: " + scorePlayer.toString()
  ...
  const collisions_speed=interval(5).pipe(
    filter(_=>isCollidePowerUp(speed_up,ball))

  )
  .subscribe(()=>{xvelocity=1.2*xvelocity})
